...
//VertexCameraBAL和VertexPointBAL:
//定义了顶点（相机位姿和特征）的类型（但是没有赋值，只有个类型）

//EdgeObservationBAL:
//重写了g2o中computeError()计算误差e的方法
//和linearizeOplus()计算雅克比的方法，
//用到了ceres::internal::AutoDiff，所以添加了cores文件夹

//g2o的初始化、顶点、边的建立，这些具体操作在g2o_bundle.cpp里实现（本文件只是负责定义和重写）
...
public:
    ...
    //误差计算：位姿误差和特征点的误差(override表示对基类虚函数的覆盖)
