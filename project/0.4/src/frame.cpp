...
{
    Vector3d p_cam = camera_->world2camera( pt_world, T_c_w_ );//3D点转换到相机坐标系坐标系
    // cout<<"P_cam = "<<p_cam.transpose()<<endl;
    if ( p_cam(2,0)<0 ) return false;//depth<0，不在像素坐标系内
    Vector2d pixel = camera_->world2pixel( pt_world, T_c_w_ );//3D点转换到uv坐标系坐标系
