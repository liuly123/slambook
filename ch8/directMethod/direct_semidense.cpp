...
int main ( int argc, char** argv )
{
    if ( argc != 2 )
    {
        cout<<"usage: usedirect_semidense path_to_dataset"<<endl;
        return 1;
    }
...
bool poseEstimationDirect ( const vector< Measurement >& measurements, cv::Mat* gray, Eigen::Matrix3f& K, Eigen::Isometry3d& Tcw )
{
    ...
    DirectBlock* solver_ptr = new DirectBlock( std::unique_ptr<DirectBlock::LinearSolverType>(linearSolver) ); //原代码不对，这是修改过的
    // g2o::OptimizationAlgorithmGaussNewton* solver = new g2o::OptimizationAlgorithmGaussNewton( solver_ptr ); // G-N
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(std::unique_ptr<DirectBlock>(solver_ptr));//这里也跟着修改
